...
- Garden (`garden/`, [garden.axelquack.de](https://garden.axelquack.de)): password-gated digital garden of markdown notes with `published: true`. Same tokens; **screening room** (this host only — see `docs/design.md`) + 16:9 picture gallery; Obsidian-style nav/search/backlinks/outline/graph/hover plus Dataview TABLE/LIST, callouts, embeds. Public Pages repo `axelquack/garden` is StatiCrypt ciphertext. See [docs/garden.md](docs/garden.md)
